...
__noinline int ebpf_search_int_page(struct bpf_xrp *context,
    ...
    for (i = 0, ii = EBPF_BLOCK_SIZE; i < nr_kv && ii > 0; ++i, --ii) {
        ...
        case EBPF_CELL_KEY_SHORT:
            ...
            break;
        default:
            bpf_printk("%d: ebpf_search_int_page: invalid cell type %d, kv %d", done, ebpf_get_cell_type(b), i);
            return -EBPF_EINVAL;
        }
        ...
        if (cmp == 0) {
            ...
            *descent_index = i + (done - 1) * 4;
            //bpf_printk("%d] 1: %d", done, prev_cell_descent_offset);
            return 1;
        } else if (cmp < 0) {
            /* user key < cell key */
            *descent_offset = prev_cell_descent_offset;
            *descent_index = i - 1 + (done - 1) * 4;
            //bpf_printk("%d] 2: %d, %d", done, i - 1 + (done - 1) * 4, nr_kv);
            return 2;
        }
    ...
    *descent_offset = prev_cell_descent_offset;
    *descent_index = i - 1 + (done - 1) * 4;
    //bpf_printk("%d] 3: %d", done, i - 1 + (done - 1) * 4);
    return 3;
}
