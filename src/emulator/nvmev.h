...
struct nvmev_io_work {
    ...
    volatile u64 subtask_result[NUM_R_CPU];
    volatile u64 subtask_status[NUM_R_CPU]; 
    volatile int done_status[NUM_R_CPU];

	volatile unsigned int next, prev;
};

#define WQ(v) (v / NR_MAX_PARALLEL_IO)
#define QE(v) (v % NR_MAX_PARALLEL_IO)

struct nvmev_io_worker {
	struct nvmev_io_work *work_queue[NUM_R_CPU];
