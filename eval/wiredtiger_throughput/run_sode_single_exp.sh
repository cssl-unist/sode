...
if [ ! -z $6 ]; then
    CACHED_DB_PATH=$6/sode_throughput_${CONFIG}_${CACHE_SIZE}_${NUM_THREADS}
fi
...
if [ $USE_XRP == 'y' ]; then
    ...
    sudo -E numactl --membind=0 --cpunodebind=0 ./run_wt $YCSB_CONFIG_PATH | tee $EVAL_PATH/result/$CONFIG-$CACHE_SIZE-cache-$NUM_THREADS-threads-sode.txt
else
    unset WT_BPF_PATH
